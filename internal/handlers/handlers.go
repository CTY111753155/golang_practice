...
// ChooseRoom takes URL parameters and chooses a room for a reservation
func (m *Repository) ChooseRoom(w http.ResponseWriter, r *http.Request) {
	roomIDStr := chi.URLParam(r, "id")
	if roomIDStr == "" {
		helpers.ServerError(w, errors.New("room ID is missing"))
		return
	}
	...
	reservation, ok := m.App.Session.Get(r.Context(), "reservation").(models.Reservation)
	if !ok {
		helpers.ServerError(w, errors.New("failed to retrieve reservation"))
		return
	}
...
// BookRoom takes URL query parameters and books a room, then redirects to the reservation page
...
// AdminPostReservationsCalendar handles post requests from the calendar
