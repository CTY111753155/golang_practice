...
func (m *postgresDBRepo) UpdateReservation(u models.Reservation) error {
	...
	_, err := m.DB.ExecContext(ctx, stmt, u.FirstName, u.LastName, u.Email, u.Phone, time.Now(), u.ID)
	if err != nil {
		return err
	}
	return nil
...
// UpdateProcessedForReservation updates the processed status for a reservation by id
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&restriction.ID, &restriction.ReservationID, &restriction.RestrictionID, &restriction.RoomID, &restriction.StartDate, &restriction.EndDate)
		if err != nil {
			return restrictions, err
		}
